...
export default class SinglePlayerTwoWindows extends Component {
  componentDidMount() {
    ...
    if (document.getElementById('codeEditor')) Store.func = document.getElementById('codeEditor').value;
  }
  getWrapperStyles() {
    return {
      height: '95vh',
  ...
  render() {
    ...
    return (
      <div style={this.getWrapperStyles()}>
        <Loop>
          <Controls onPlay={this.props.onPlay} onPause={this.props.onPause} onEnd={this.props.onEnd} />
          <div id={'game0'} style={this.getGameWrapperStyles()}>
            <Stage style={this.getGameStyles()}>
              <Tile tiles={this.props.gameData.config.game1.tiles} />
              <div>
                {this.props.gameData.config.game1.obstacles.map((obstacle, index) => {
          ...
          <div id={'game1'} style={this.getGameWrapperStyles()}>
            <Stage style={this.getGameStyles()}>
              <Tile tiles={this.props.gameData.config.game2.tiles} />
              <div>
                {this.props.gameData.config.game2.obstacles.map((obstacle, index) => {
                  return <Obstacle key={index} obstacle={obstacle} index={index} />;
                })}
              </div>
              <Collectives
