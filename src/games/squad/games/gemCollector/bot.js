...
export default class Character extends Component {
  ...
  loop = () => {
    ...
    if (Store.mode === 'play') {
      if (Util.rect2parent(player, parentEl, direction)) Store.moveCharacter(this.props.charId);
      var world = {
      ...
      } else if (this.props.player1Function) var setDirection = this.props.player1Function(world);
      else if (this.props.player2Function) var setDirection = this.props.player2Function(world);
      else var setDirection = this.props.getCommands(world);
      if (setDirection) {
        if (setDirection.left) Store.changeDirection(this.props.charId, 'left');
        else if (setDirection.right) Store.changeDirection(this.props.charId, 'right');
        else if (setDirection.up) Store.changeDirection(this.props.charId, 'up');
        else if (setDirection.down) Store.changeDirection(this.props.charId, 'down');
      }
    }
    this.getCollectives();
  ...
  render() {
    switch (this.props.type) {
      case 'gnome1':
        return (
          <div id={'bt' + this.props.charId}>
            <Gnome1 position={Store.position[this.props.charId]} direction={Store.direction[this.props.charId]} />
          </div>
        );
      case 'gnome2':
        return (
          <div id={'bt' + this.props.charId}>
            <Gnome2 position={Store.position[this.props.charId]} direction={Store.direction[this.props.charId]} />
          </div>
        );
      case 'blonde':
        return (
          <div id={'bt' + this.props.charId}>
            <Blonde position={Store.position[this.props.charId]} direction={Store.direction[this.props.charId]} />
          </div>
        );
      case 'brunette':
        return (
          <div id={'bt' + this.props.charId}>
            <Brunette position={Store.position[this.props.charId]} direction={Store.direction[this.props.charId]} />
          </div>
        );
      case 'drone1':
        return (
          <div id={'bt' + this.props.charId}>
            <Drone1 position={Store.position[this.props.charId]} direction={Store.direction[this.props.charId]} />
          </div>
        );
      case 'drone2':
        return (
          <div id={'bt' + this.props.charId}>
            <Drone2 position={Store.position[this.props.charId]} direction={Store.direction[this.props.charId]} />
          </div>
        );
      case 'drone3':
        return (
          <div id={'bt' + this.props.charId}>
            <Drone3 position={Store.position[this.props.charId]} direction={Store.direction[this.props.charId]} />
          </div>
        );
      default:
        return (
